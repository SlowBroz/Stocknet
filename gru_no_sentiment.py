...
if len(train_data) == 0 or len(val_data) == 0 or len(test_data) == 0:
    raise ValueError(f"Ingen data for aksjen {target_symbol}")
...
plt.title(f'Stock Price Prediction for {target_symbol} (GRU, no sentiment)')
plt.xlabel('Date')
plt.ylabel('Price')
plt.legend()
plt.xticks(rotation=45)
